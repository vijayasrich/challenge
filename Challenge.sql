...
select 
    ...
    isnull(sum(d.donationamount), 0) as total_donation_amount
from 
    shelters s
left join 
    donations d on s.shelterid = d.shelter_id
group by 
    s.name
order by 
    s.name
...
select count(distinct participants.participantid) as total_participants
from participants
...
where shelters.location like '%Vizag%'
select * from Participants
select * from AdoptionEvents
...
from pets
join adoptionevents on pets.petid = adoptionevents.eventid
join participants on adoptionevents.eventid = participants.participantid
...
select top 3 shelters.name as shelter_name, count(adoptionevents.petid) as adopted_pet_count
from shelters
